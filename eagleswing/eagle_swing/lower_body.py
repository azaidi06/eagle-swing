...
def _compute_lower_body_metrics(
    ...
    # # --- FIX: Enforce consistency to prevent L/R swaps ---
    # for i in range(1, len(l_hip_s)):
    #     # Calculate vector from Right to Left Hip for previous and current frame
    #     prev_vec = l_hip_s[i-1] - r_hip_s[i-1]
    #     curr_vec = l_hip_s[i] - r_hip_s[i]
        
    #     # Calculate dot product. If < 0, the vector flipped > 90 degrees.
    #     # Physically impossible for hips between frames -> implies labeling swap.
    #     if np.dot(prev_vec, curr_vec) < 0:
    #         # Swap Hips
    #         l_hip_s[i], r_hip_s[i] = r_hip_s[i].copy(), l_hip_s[i].copy()
            
    #         # Swap Knees and Ankles (assuming full leg swap)
    #         l_knee_s[i], r_knee_s[i] = r_knee_s[i].copy(), l_knee_s[i].copy()
    #         l_ankle_s[i], r_ankle_s[i] = r_ankle_s[i].copy(), l_ankle_s[i].copy()
    # # -------------------------------------------------------
